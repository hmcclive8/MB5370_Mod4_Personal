...
```{r}
# 1. Line chart = geom_line(), Boxplot = geom_boxplot(), Area chart = geom_area()
...
These two graphs will look IDENTICAL because they both plot the same dataset (mpg) using the same aesthetics (x = displ, y = hwy), and the same geoms (geom_point() for scatter points and geomsmooth() for the trend line)
